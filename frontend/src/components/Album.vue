        ...
        .v-card__text
          .container.grid-list-md.pt-0
            .layout.column.justify-center
              v-form(ref="form" v-model="formValid" @submit="")
                .flex.pa-0()
...
export default {
    ...
    busyVal(val) {
      if (val) {
        loading = this.$loading.show(
          ...
          {
            default: this.$createElement(Loading)
          }
        );
      } else loading.hide();
    }
  ...
  mounted() {
    if (this.was) this.model = JSON.parse(JSON.stringify(this.was));
  },
  computed: {
    valid() {
  ...
  methods: {
    ...
    async processPhotos() {
      ...
      for (var photo of this.model.photos) {
    ...
    async upload() {
      ...
      if (response.status === 200) {
        ...
        this.$dialog.message.error(response.data, {
          position: "top"
        });
    },
    async update() {
